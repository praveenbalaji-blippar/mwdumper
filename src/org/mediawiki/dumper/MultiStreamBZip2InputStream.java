...
public class MultiStreamBZip2InputStream extends CompressorInputStream {
  ...
  public MultiStreamBZip2InputStream(final InputStream in) throws IOException {
    fInputStream = in;
  ...
  public int read() throws IOException {
    ...
    if(ch == -1) {
      /*
       * If this is a multistream file, there will be more data that follows
       * that is a valid compressor input stream. Restart the decompressor
  ...
  /**
   * Read the data from read(). This makes sure we funnel through read so we can
   * do our multistream magic.
   */
  @Override
  public int read(final byte[] dest, final int off1, final int len) throws IOException {
    int off = off1;
